    ...
    setTimeout(async () => {
      try {
        ...
        const populateQuote = (quote: any, db: any) => {
          ...
          const populatedItems = (quote.items || []).map((item: any, index: number) => {
            ...
            return {
              ...
              } : { name: `Produto não encontrado (${pid})`, images: [] }
            };
          });
        ...
        if (url.match(/^\/products\/[^/]+$/) && method === 'get') {
          const id = url.split('/').pop();
          const item = db.products.find(p => String(p.id) === String(id));
          return item ? response(200, item) : error(404, 'Product not found');
        }
        if (url === '/products' && method === 'post') {
          const prod = { id: generateId(), ...data };
          db.products.push(prod);
          saveDb(db);
          return response(201, prod);
        }
        if (url.match(/^\/products\/[^/]+$/) && method === 'post') { 
          const id = url.split('/').pop();
        ...
        // Updating Quote Items (Admin)
        if (url.match(/^\/admin\/quotes\/[^/]+\/items$/) && method === 'put') {
           console.log('[Mock API] Updating items for quote:', data.items);
           const qid = url.split('/')[3];
           const quote = db.quotes.find(q => q.id === qid);
           if (quote) {
        ...
        // Updating General Quote Info (Admin)
        if (url.match(/^\/admin\/quotes\/[^/]+$/) && method === 'put') {
           console.log('[Mock API] Updating quote meta:', data);
           const qid = url.split('/').pop();
           const quote = db.quotes.find(q => q.id === qid);
           if (quote) {
             if (data.status) quote.status = data.status;
             if (data.admin_notes !== undefined) quote.admin_notes = data.admin_notes;
             if (data.total_estimated_value !== undefined) quote.total_estimated_value = data.total_estimated_value;
             saveDb(db);
             return response(200, populateQuote(quote, db));
           }
        }
        ...
        return reject({ response: { status: 500, data: { message: 'Mock Error' } } });
      }
    }, 300);
  });
};
